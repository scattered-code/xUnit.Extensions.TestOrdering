    ...
    public sealed class DependencyTestCollectionOrderer : ITestCollectionOrderer {
        ...
        private sealed class TestCollectionCache {
            ...
            private static void CreateCollectionTypeMap(Dictionary<string, Type> collectionTypeMap) {
                ...
                var libs = DependencyContext.Default.RuntimeLibraries.Select(info => System.Reflection.Assembly.Load(new AssemblyName(info.Name)));
                
                
                var types = 
                    from assembly in libs
                    from type in assembly.GetExportedTypes()
                
                    // Xunit accesses attribute data by constructor arguments
                    let collectionAttrData = type.GetTypeInfo().CustomAttributes.FirstOrDefault(x => x.AttributeType == typeof(CollectionAttribute))
